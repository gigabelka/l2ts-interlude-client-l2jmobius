...
const ServerConfigSchema = z.object({
    ...
    loginPort: z.number().int().min(1).max(65535, "Port must be between 1 and 65535"),
    gamePort: z.number().int().min(1).max(65535, "Port must be between 1 and 65535"),
    protocol: z.number().int().min(1).max(9999, "Protocol must be a positive integer"),
    serverId: z.number().int().min(1, "Server ID must be at least 1"),
    charSlotIndex: z.number().int().min(0).max(7, "Character slot must be between 0 and 7"),
});
