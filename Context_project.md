...
> Дата: 2026-03-20 (обновлено: **Финальная проверка и полировка** ✅)
...
| `disconnected` | Отключение от сервера | ✅ **Новое** |
...
- `disconnected` — при отключении от game server | ✅ **Новое**
...
- ✅ Событие `disconnected` при отключении от сервера | **Новое**
...
```typescript
{
  ...
  gameState?: GameState;  // ✅ Добавлено для сброса состояния при отключении
}
```
...
**Сброс GameState при отключении (GameClient.ts):**
```typescript
...
## ✅ Финальная проверка (2026-03-20)

### Выполненные проверки:

| Проверка | Статус | Примечания |
|----------|--------|------------|
| TypeScript компиляция | ✅ Пройдена | `npx tsc --noEmit` — ошибок нет |
| Циклические зависимости | ✅ Не обнаружены | Между GameState/GameStateUpdater/WsServer/index.ts |
| Тесты | ✅ 168 тестов | Все тесты проходят успешно |
| Сборка | ✅ Успешна | `npm run build` — без ошибок |
| Graceful shutdown | ✅ Реализован | `SIGINT`/`SIGTERM` обработчики в index.ts |
| Edge case: WS до входа | ✅ Обработан | snapshot с `me: null` |
| Edge case: Game server disconnect | ✅ Обработан | `GameState.reset()` + событие `disconnected` |
| Edge case: Невалидный JSON | ✅ Обработан | try/catch в `onMessage` |
| Edge case: 100 NPC | ✅ Обработан | Throttling 100ms на объект |

### Список изменённых/созданных файлов:

**WebSocket API (новые файлы):**
| Файл | Описание |
|------|----------|
| `src/ws/WsServer.ts` | WsApiServer — WebSocket сервер с throttling/batching |
| `src/ws/HttpEndpoints.ts` | HTTP endpoints для снимков GameState |
| `src/ws/auth.ts` | Аутентификация с timing-safe comparison |

**GameState (новые файлы):**
| Файл | Описание |
|------|----------|
| `src/game/GameState.ts` | Единое хранилище состояния игры |
| `src/game/GameStateUpdater.ts` | Мост между пакетами и GameState |
| `src/game/entities/types.ts` | TypeScript интерфейсы для API |
| `src/game/dictionaries/classNames.ts` | Словарь имён классов |
| `src/game/dictionaries/npcNames.ts` | Словарь имён NPC |
| `src/game/dictionaries/itemNames.ts` | Словарь имён предметов |
| `src/game/dictionaries/index.ts` | Экспорты словарей |

**Тесты (новые файлы):**
| Файл | Описание |
|------|----------|
| `src/__tests__/GameState.test.ts` | Тесты GameState |
| `src/__tests__/GameStateUpdater.test.ts` | Тесты GameStateUpdater |
| `src/__tests__/WsServer.test.ts` | Тесты WsApiServer |

**Примеры (новые файлы):**
| Файл | Описание |
|------|----------|
| `examples/ws-client-node.js` | WebSocket клиент на Node.js |
| `examples/ws-client-python.py` | WebSocket клиент на Python |

**Изменённые файлы:**
| Файл | Изменения |
|------|-----------|
| `src/index.ts` | Добавлена инициализация WebSocket API, graceful shutdown |
| `src/config.ts` | Добавлена конфигурация WS_CONFIG |
| `src/config/di/composition.ts` | Добавлены GameState, GameStateUpdater, WsApiServer в DI |
| `src/config/di/Container.ts` | Добавлены токены DI_TOKENS.GameState, GameStateUpdater, WsApiServer |
| `src/game/GameClient.ts` | Добавлена зависимость gameState, сброс при отключении |
| `.env.example` | Добавлены переменные WebSocket API |
| `readme.md` | Обновлена документация с WebSocket Vision API |

### Итог:

WebSocket Vision API полностью интегрирован и готов к использованию. Все проверки пройдены, все тесты работают. Система корректно обрабатывает отключения от сервера и отправляет событие `disconnected` всем подключенным WebSocket клиентам.
